...
int volumeBuzzer = 0; //VARIÁVEL QUE ARMAZENA O VALOR DA LUMINOSIDADE QUE SERÁ APLICADA AO LED
   ...
   ADCSRA |= (1 << ADSC);
    ...
    int v = (ADCH << 8) + v;  
     ...
     // leituraA0 = analogRead(pinoPOT);//LÊ O VALOR NO PINO ANALÓGICO (VALOR LIDO EM BITS QUE VAI DE 0 A 1023)
      volumeBuzzer = map(v , 0, 1023 , 0, 255); //EXECUTA A FUNÇÃO "map" DE ACORDO COM OS PARÂMETROS PASSADOS
    ...
    //  analogWrite(pinoBuzzer, volumeBuzzer); //APLICA AO BUZZER O VALOR DE CONTROLE (PWM) GERADO PELA FUNÇÃO "map"
      digitalWrite (pinoBuzzer, volumeBuzzer); //APLICA AO BUZZER O VALOR DE CONTROLE (PWM) GERADO PELA FUNÇÃO "map"
